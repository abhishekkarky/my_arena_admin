...
import { getCountryCountForUserApi } from '../../../../apis/api';
...
const UserGeoChart = () => {
    ...
    useEffect(() => {
        getCountryCountForUserApi().then((res) => {
            if (res.data.success) {
    ...
    const chartData = [["Country", "Total Users"], ...counts.map(({ country, count }) => [country, count])];
            ...
            <div className='w-full h-auto max-h-[60vh] bg-white rounded-lg shadow-md border'>
                {
                    isLoading ?
                        ...
                        <div className="flex flex-col p-4 gap-6 rounded-lg bg-white w-full h-full shadow-md overflow-hidden">
                            <p className='text-neutral-700 text-xl'>Overall <span className='text-green-600'>users</span> in each country</p>
                            <Chart
